...
4. `.shipit/HANDOFF.md` — context from previous tasks (if it exists)

The HANDOFF.md file contains a cumulative log of what previous tasks accomplished, including files changed, key decisions, and context you need. **Read it carefully** to understand what has already been done and avoid conflicting with previous work.

## Process

1. **Find your task** — Read STATE.md to get `current_task` number, find that task in PLAN.md
2. **Review handoff** — Read HANDOFF.md to understand what previous tasks did, what decisions were made, and what context you need
3. **Understand context** — Read the files listed in the task
4. **Execute with TDD** (if task has TDD: yes):
...
5. **Execute without TDD** (if task has TDD: no):
   a. Make the change
   b. Verify it works (run relevant commands)
6. **Commit** — Atomic commit with descriptive message
7. **Append to HANDOFF.md** — Add a summary of this task (see Handoff Format below)
8. **Update STATE.md**:
...
After completing a task, append this to `.shipit/HANDOFF.md`:

```markdown
## Task N: <task name> ✅
- **Files changed:** <list of files you created or modified>
- **What was done:** <1-2 sentence summary of the work>
- **Key decisions:** <any decisions you made and why>
- **Context for next tasks:** <anything the next task needs to know>
```

Keep each entry concise (4-5 lines). Do NOT rewrite the entire file — only append your task's entry at the end.
